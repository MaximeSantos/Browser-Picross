 ...
 * @returns {Number|Null} Returns -1 for an empty cell on a middle click, 0 for a crossed cell on a right click and 1 for a full cell on a left click.
 */
export default function getAction(event: string, button: number): number | null {
	if (event == 'mousedown') {
		...
		return button == 0 ? 1 : button == 2 ? 0 : button == 1 ? -1 : null;
	} else if (event == 'mouseover') {
		// Left click is 1 / Right click is 2 / Middle click is 4
		return button == 1 ? 1 : button == 2 ? 0 : button == 4 ? -1 : null;
	}
	return null;
}
